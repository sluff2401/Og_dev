...
def post_new(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            post.save()
            return redirect('ogevents.views.post_detail', pk=post.pk)
    else:
        form = PostForm()
...
def post_draft_list(request):
    ...
    return render(request, 'ogevents/post_draft_list.html', {'posts': posts})

@login_required
def post_publish(request, pk):
    post = get_object_or_404(Post, pk=pk)
    post.publish()
    return redirect('ogevents.views.post_detail', pk=pk)
