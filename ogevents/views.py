...
from .forms                               import EventForm, UserdetailForm
  ...
  if request.user.is_authenticated():
    ...
    for eventx in eventxs:
      if user.is_superuser            == True:
        ...
        editable                      = '1'
      elif eventx.schememanager       == user:
        editable                      = '1'
      else:
        editable                      = '0'
  ...
  if request.method              == "POST":
    ...
    if form.is_valid():
      ...
      if event.event_date < timezone.localtime(timezone.now()).date():
        return render(request, 'ogevents/event_input_form.html', {'form': form})
      user                       = User.objects.get(id=request.user.id)
      #userdetail                 = user.userdetail
